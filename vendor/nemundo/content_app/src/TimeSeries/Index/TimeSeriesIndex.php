...
trait TimeSeriesIndex
{
    ...
    private function saveDayPeriod(Date $date)
    {
        ...
        if (!isset($this->periodDayList[$date->getIsoDateFormat()])) {
            ...
            $id->filter->andEqual($id->model->date, $date->getIsoDateFormat());
            $periodId = $id->getId();

            $this->periodDayList[$date->getIsoDateFormat()] = $periodId;

        }

        return $this->periodDayList[$date->getIsoDateFormat()];
